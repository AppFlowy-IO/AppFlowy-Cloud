...
pub async fn get_user_workspace_structure(
  ...
  let folder = get_latest_folder_collab(group_manager, collab_storage, &uid, &workspace_id).await?;
  Ok((&folder).into())
}

pub async fn get_latest_folder_collab(
  group_manager: AppStateGroupManager,
  collab_storage: Arc<CollabAccessControlStorage>,
  uid: &i64,
  workspace_id: &str,
  ...
  let encoded_collab =
    get_latest_folder_collab_encoded(group_manager, collab_storage, uid, workspace_id).await?;
...
pub async fn get_latest_folder_collab_encoded(
  group_manager: AppStateGroupManager,
  collab_storage: Arc<CollabAccessControlStorage>,
  uid: &i64,
  workspace_id: &str,
